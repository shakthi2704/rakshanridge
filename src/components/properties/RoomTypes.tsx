                        ...
                        <Reveal key={room.key} delay={index * 100}>
                            <div className="flex h-full flex-col bg-paper">
                                ...
                                <div className="flex flex-1 flex-col p-6">
                                    <Link href={`/properties/${propertySlug}/${slugifyRoomKey(room.key)}`}>
                                        <h3 className="font-serif text-xl font-medium text-ink hover:text-navy">
                                            {room.name}
                                        </h3>
                                    </Link>

                                    <div className="mt-3 flex items-center gap-5">
                                        <span className="flex items-center gap-2 font-sans text-xs tracking-[0.05em] text-slate uppercase">
                                    ...
                                    <div className="mt-6 flex items-center justify-between border-t border-navy/10 pt-5">
                                        <span className="font-sans text-xs tracking-[0.1em] text-navy uppercase">
                                            {room.priceLabel}
                                        </span>
                                        <a
                                            href={`/book?room=${room.key}`}
                                            className={buttonVariants({ variant: "outline-navy", size: "sm" })}
                                        >
                                            {bookLabel}
                                        </a>
                                    </div>
